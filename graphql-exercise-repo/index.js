...
const resolvers = {
  Query: {
		...
		allBooks: (root, args) => {
			...
			let bookArray = books;
			if (args.author) {
				bookArray = bookArray.filter(book => book.author == args.author);
			}
			if (args.genre) {
				bookArray = bookArray.filter(book => book.genres.includes(args.genre));
			}
			return bookArray;
		},
		allAuthors: () => authors.map((author) => {
			const bookCount = books.reduce((accumulator, book) => {
			return book.author === author.name ? accumulator + 1 : accumulator
		}, 0)
	...
	Mutation: {
		addBook: (root, args) => {
			...
			const authorExists = authors.find(author => author.name === book.author);
			if (!authorExists) {
				const author = {
					name: book.author,
					id: uuid()
